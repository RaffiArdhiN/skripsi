...
# Cooldown antar kombinasi KD — beda dengan saran 15-30 menit dari pembimbing.
# Dipilih 60s sebagai kompromi: cukup untuk CUDA benar2 selesai free memory,
# tapi tidak membuang waktu berjam-jam untuk 18 kombinasi tersisa.
# Kalau hang TERULANG meski cleanup+cooldown ini sudah jalan, itu sinyal kuat
# akar masalahnya BUKAN VRAM leak — kemungkinan disk penuh, driver crash,
# atau isu level sistem lain. Naikkan ke 900-1800 hanya kalau mau coba
# rekomendasi pembimbing secara literal.
COOLDOWN_SECONDS = 60

def log_gpu_memory(tag=""):
    """Cetak VRAM allocated & reserved — buat diagnosis kalau hang terulang."""
    if DEVICE == 'cuda':
        alloc = torch.cuda.memory_allocated() / 1024**3
        reserv = torch.cuda.memory_reserved() / 1024**3
        suffix = f" [{tag}]" if tag else ""
        print(f"  → VRAM{suffix}: allocated={alloc:.2f}GB | reserved={reserv:.2f}GB")
...
def make_loaders(augment=False, batch_size=STUDENT_BATCH,
                  train_workers=NUM_WORKERS, test_workers=NUM_WORKERS):
    # val loader: num_workers=0 — HANYA dipakai untuk early-stopping check
    # per epoch, TIDAK pernah dipakai di measure_fps(). Aman dikurangi.
    #
    # test loader: TETAP num_workers=test_workers (default 4) — loader ini
    # dipakai LANGSUNG di measure_fps(). Kombinasi grid search sebelumnya
    # sudah terukur FPS-nya dengan num_workers=4 (ada overlap data-loading
    # vs compute yang "menyembunyikan" waktu decode video di balik forward
    # pass). Kalau diubah ke 0 sekarang, FPS kombinasi berikutnya TIDAK
    # comparable lagi dengan kombinasi sebelumnya — metodologi pengukuran
    # jadi beda diam-diam. Jadi worker count cuma dipotong dari train+val
    # (8 dari 12), test tetap penuh demi konsistensi metrik FPS.
    train_kw = dict(batch_size=batch_size, num_workers=train_workers, pin_memory=True)
    val_kw   = dict(batch_size=batch_size, num_workers=0,             pin_memory=True)
    test_kw  = dict(batch_size=batch_size, num_workers=test_workers,  pin_memory=True)
    return (
...
if SKIP_TO_SCENARIO <= 1:
    ...
    # FPS sengaja TIDAK diukur di sini — diukur terpisah belakangan via
    # fps_measurement.py supaya semua skenario (Teacher/Student/KD) diukur
    # dalam kondisi sistem yang seragam (suhu GPU, beban user lain, batch
    # size yang sama). Lihat catatan di fps_measurement.py untuk detail.
    fps_t = 'PENDING'

    print(f"\n  Teacher | val_acc={val_acc_t:.4f} | "
          f"test_acc={test_acc_t:.4f} | FPS={fps_t}")
    print(rp)
...
# --- Bersihkan student_base dari VRAM sebelum KD grid search ---
# (kalau Skenario 2 baru saja dilatih, instance-nya masih ada di memory)
if 'student_base' in locals():
    del student_base, optimizer_sb, scheduler_sb
    torch.cuda.empty_cache()
    gc.collect()
    print("  → student_base dibersihkan dari VRAM")
    log_gpu_memory("setelah cleanup student_base")
...
best_kd = {'f1': 0, 'config': ''}
combo_num = 0
student_kd = None  # tracker instance sebelumnya, untuk cleanup eksplisit
...
log_gpu_memory("awal Skenario 3")
...
for t_type, alpha, kd_temp in itertools.product(
    ...
    # --- Bersihkan instance kombinasi SEBELUMNYA sebelum buat yang baru ---
    # Ini bagian inti fix: model lama di-delete eksplisit + cache di-clear,
    # bukan cuma reassign variabel (yang gc-nya bisa telat di CUDA tensor).
    # Loader juga dibersihkan — DataLoader dengan num_workers>0 spawn subprocess
    # OS yang idealnya di-terminate sebelum loader baru (dan worker barunya) dibuat.
    if student_kd is not None:
        del student_kd, optimizer_kd, scheduler_kd
        del train_loader, val_loader, test_loader
        torch.cuda.empty_cache()
        gc.collect()
        log_gpu_memory(f"setelah cleanup combo {combo_num-1}")

    print(f"\n[{combo_num}/{total_kd}] {config_name}")
    log_gpu_memory(f"sebelum combo {combo_num}")
    ...
    save_results()
    log_gpu_memory(f"selesai combo {combo_num}")

    if COOLDOWN_SECONDS > 0:
        print(f"  → Istirahat {COOLDOWN_SECONDS}s sebelum kombinasi berikutnya...")
